...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    const text = "bhushan776767@gmail.com";
    navigator.clipboard.writeText(text);
    setCopied(true);
  };

  const chunkedLogos = logos ? logos.reduce((result, logo, index) => {
    const chunkIndex = Math.floor(index / 5);
    if (!result[chunkIndex]) {
      result[chunkIndex] = [];
    }
    result[chunkIndex].push(logo);
    return result;
  }, [] as { img: string; title: string }[][]) : [];
  ...
  />
        ...
        {id === 5 && chunkedLogos.length > 0 && (
          <div className="absolute inset-0 mt-10  flex flex-col items-center">
            {chunkedLogos.map((logoRow, rowIndex) => (
