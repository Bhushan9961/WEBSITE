...
//     const arcs = data;
//     const points = arcs.reduce((acc, arc) => {
//       const rgb = hexToRgb(arc.color) as { r: number; g: number; b: number };
//       acc.push({
//         size: defaultProps.pointSize,
//         order: arc.order,
...
// Update the _buildData function with type-safe object indexing
const _buildData = useCallback(() => {
  const arcs = data;
  const points = arcs.reduce((acc, arc) => {
    const rgb = hexToRgb(arc.color) as { r: number; g: number; b: number };
    ...
    return acc;
  }, [] as Array<{ size: number; order: number; color: (t: number) => string; lat: number; lng: number }>);

  // Helper function to ensure type safety when accessing keys
  const isLatLngKey = (key: string): key is keyof { lat: number; lng: number } => {
  ...
  const filteredPoints = points.filter(
    (v, i, a) =>
      a.findIndex((v2) =>
        ["lat", "lng"].every((k) => isLatLngKey(k) && v2[k] === v[k])
      ) === i
  );
